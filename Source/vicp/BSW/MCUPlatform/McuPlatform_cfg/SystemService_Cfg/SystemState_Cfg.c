...
extern void M4Msg_Init(void);
extern void M4Msg_DeInit(void);
...
void SysState_apResetContainer(void)
{
    ...
    Sig_DioInit();
    EED_ResetInit();
    SocDriver_S0Init();		//eSocPowerState = E_DEV_SPOWEROFF
    Application_S0Init();
...
{ 
	...
	CanDriver_Init();
	SocDriver_Init();
	SocMsg_Init();		//DTA_Reset();EnableReq=E_NO_CONTRONL,bDrivetErrorr =FALSE;
	SocManager_Init();	//eSocState = E_SOC_IDLE;bSocConnectedstate = E_SOC_CONNECTTED_STARTED
	SocTransmitManager_Init();
	Dtc_Init();
	CanDiagDtcInit();
	Soc_Menual_Udata();//GPIO_SetInputMode(GPIO_PORT_8,GPIO_PIN_6, 0);//for test trun SOC UDDATA
	#ifdef __DEBUG__
    CpuRate_Init();
	#endif
...
//	PowerManager_DeInit();
	...
	SocMsg_DeInit();
	SocDriver_DeInit();
//	Radar_TAU_Timer_Interrupt_DeInit();
//	Radar_TAU_Capture_DeInit();
	//M4UartDriver_Close();
	M4Msg_DeInit();
}
