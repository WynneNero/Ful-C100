...
{  
    if((AP_PORT_0<=port) && (port< AP_PORT_1)) /*set APORT*/
    {
        APORT0_REG_APBDC(port - AP_PORT_0) &=(~(1u<<pin));
        if(E_INPUT_HIZ!=Mode)
            APORT0_REG_APIBC(port - AP_PORT_0)|=(1u<<pin);
        else
          // APORT_REG_APIBC(port - AP_PORT_0)&=(~(1u<<pin));
        //APORT_REG_APM(port - AP_PORT_0) |= (U16)(1u<<pin);
        APORT0_REG_APIBC(port - AP_PORT_0)&=(~(1u<<pin));
		APORT0_REG_APM(port - AP_PORT_0) |= (U16)(1u<<pin);
    }
   // else if(JP_PORT_0 <= port) /*set JPORT*/
	else  if((AP_PORT_1<=port) && (port< JP_PORT_0)) /*set APORT*/
    {
//        /*initialize*/
...
//        /*change value*/
			APORT1_REG_APBDC(port - AP_PORT_0) &=(~(1u<<pin));
			if(E_INPUT_HIZ!=Mode)
			APORT1_REG_APIBC(port - AP_PORT_0)|=(1u<<pin);
			else
			APORT1_REG_APIBC(port - AP_PORT_0)&=(~(1u<<pin));
			APORT1_REG_APM(port - AP_PORT_0) |= (U16)(1u<<pin);
	}
 ...
 if((AP_PORT_0<=port) && (port< AP_PORT_1)) /*set APORT*/
  {
      APORT0_REG_APBDC(port - AP_PORT_0) &=(~(1u<<pin));
      APORT0_REG_APIBC(port - AP_PORT_0)  &=(~(1u<<pin));
      APORT0_REG_APM(port - AP_PORT_0)&=(U16)(~(1u<<pin)); /*0-OUTPUT*/
      return ;
  }
  //else if(JP_PORT_0 <= port) /*set JPORT*/
  else if((AP_PORT_1<=port) && (port< JP_PORT_0)) /*set APORT*/
...
//      if(Mode==E_OPEN_DRAIN ) /*out type*/
	   APORT1_REG_APBDC(port - AP_PORT_0) &=(~(1u<<pin));	  
	   APORT1_REG_APIBC(port - AP_PORT_0)  &=(~(1u<<pin));	  
	   APORT1_REG_APM(port - AP_PORT_0)&=(U16)(~(1u<<pin)); /*0-OUTPUT*/	  
	   return ;	  
   }
...
void GPIO_SetAlterMode( U8 port, U8 pin,  GPIO_ALTERMODE AlterMode, U8 options )
{
    ...
    if((AP_PORT_0<=port) && (port< AP_PORT_1))		
	{	 
	  APORT0_REG_APIBC(port - AP_PORT_0)&=(~(1u<<pin));	 
	  APORT0_REG_APM(port - AP_PORT_0) |= (U16)(1u<<pin);	 
	  ...
	  APORT0_REG_APM(port - AP_PORT_0) &= ~(1u<<pin); /*direction output*/
	}
	else if((AP_PORT_1<=port) && (port< JP_PORT_0))
	 
    {
        ...
        if(!(options&GPIO_OPT_INPUT) || (options&GPIO_OPT_BIDIRECT) )
          APORT1_REG_APM(port - AP_PORT_0) &= ~(1u<<pin); /*direction output*/
    }
    else if(JP_PORT_0 <= port)
    {
...
//    if((AP_PORT_0<=port) && (port< JP_PORT_0)) /*APORT*/
	U8 Ret=0u;	 
	 if((AP_PORT_0<=port) && (port< AP_PORT_1)) /*APORT*/	 
	  {	 
	  Ret=(U8)((APORT0_REG_PPR(port - AP_PORT_0)&((U16)(1u<<pin)))>>pin);	 
	 }	 
	else if((AP_PORT_1<=port) && (port< JP_PORT_0)) /*APORT*/	 
    {
        Ret=(U8)((APORT1_REG_PPR(port - AP_PORT_0)&((U16)(1u<<pin)))>>pin);
    }
...
void GPIO_SetVal( U8 port, U8 pin, BOOL isHlevel)
{
	...
	{	
	  		if(isHlevel)	
	 	{	
	 		 APORT0_REG_P(port - AP_PORT_0) |=((U16)(1u<<pin));	
	  	}	
	  else	
	  	{	
	 		APORT0_REG_P(port - AP_PORT_0) &=(~((U16)(1u<<pin)));	
	  ...
	  else if((AP_PORT_1<=port) && (port< JP_PORT_0)) /*APORT*/ 
    {
      if(isHlevel)
      {
          APORT1_REG_P(port - AP_PORT_0) |=((U16)(1u<<pin));
      }
      else
      {
          APORT1_REG_P(port - AP_PORT_0) &=(~((U16)(1u<<pin)));
...
void GPIO_SetDriverStrengthDrain_Port(U8 port,  U32 pdscval)
{
	...
	if(protCmd &&(GPIO_PORT_8!=port) &&(GPIO_PORT_9!=port) &&(GPIO_PORT_1!=port) &&(GPIO_PORT_2!=port)
	&&(GPIO_PORT_18!=port) &&(GPIO_PORT_20!=port)) /*no this register*/	 
    {
...
*FUNCTION : U8 Get_GPIO_Val(U8 G_port, U8 G_pin)
*DESCRIPTION : used for HeFei ShengTaike to control GPIO
*RETURN : CHAR

*****************************************************/
U8 Get_GPIO_Val(U8 G_port, U8 G_pin)
{
	GPIO_GetVal( G_port, G_pin );
}

/****************************************************

*FUNCTION : void Set_GPIO_Val(U8 S_port, U8 S_pin, BOOL S_isHlevel)
*DESCRIPTION : used for HeFei ShengTaike to control GPIO
*RETURN : void

*****************************************************/

void Set_GPIO_Val(U8 S_port, U8 S_pin, BOOL S_isHlevel)
{
	GPIO_SetVal( S_port, S_pin, S_isHlevel);
}
