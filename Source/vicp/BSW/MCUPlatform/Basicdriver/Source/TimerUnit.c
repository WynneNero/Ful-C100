...
//ALL_SWITCH_STRUCT control_all_switch;//5358
...
{ /*Timer Unit     channel        interruput id */
    ...
    INTTAUB0I0, INTTAUB0I1, INTTAUB0I2, INTTAUB0I3_PWGA16, INTTAUB0I4,INTTAUB0I5, INTTAUB0I6, INTTAUB0I7,
    INTTAUB0I8, INTTAUB0I9_PWGA19, INTTAUB0I10, INTTAUB0I11_PWGA26, INTTAUB0I12, INTTAUB0I13, INTTAUB0I14, INTTAUB0I15,
    #endif
...
U32 TAUB_BASEADDR_1(TAU_UNIT un)
{
    U32 addr=0;    
    switch(un)
    {
 ...
 /* base address of all TAUB defines  */
const U32 TAUB_BASEADDR[TAU_U_MAX]=
{
#if (MCU_PIN>=PIN100)
	 TAUB0_REG_BASEADD,
#endif
...
/******************************************************************************
* Function Name : TAU_Capture_Init
* Description   : do	the configrations of a capture channel
* Argument	 : un  TAU_UNIT, select timer array unit
* 					 chm  TAU_CHANNEL. for TAUJ, TAU_CH0~TAU_CH3
* 								 for TAUB and TAUD, TAU_CH0~TAU_CH15
* 					  edge	
* 							  E_TAUB_TIS_EDGE_FALL_RISE ,low-width measurement	
* 							  E_TAUB_TIS_EDGE_RISE_FALL ,high-width measurement
*
*  ck fre ,TAU_GetClolckFre_Channel()
*  input pulse width: 
*    if TAUJ	max_s= (1/ck fre)s *(0xffffffff+1)
...
* Return Value  : none
******************************************************************************/ 
BOOL TAU_Capture_Init (TAU_UNIT un, TAU_CHANNEL chm, U8 edge)
{
 //    U8 int_id=0;
	 assert_param(IS_TAU_CHANNEL(un,chm));
	 assert_param(isUnusedTAUChannel(un,chm));
	 ...
	 {
	   DNFATAUB0ICTL		  = 0x00u;	
	   DNFATAUB0IEN 		  |= (U16)(1<<chm); // 1enable
	 }
	 else
#endif
	 if(TAUD0_U2==un)
	 {
	   DNFATAUD0ICTL		  = 0x00u;	
	   DNFATAUD0IEN 		  |= (U16)(1<<chm);
	 }
#if(MCU_PIN==PIN176)
	 else if(TAUB1_U1==un)
	 {
	   DNFATAUB1ICTL		  = 0x00u;	
	   DNFATAUB1IEN 		  |= (U16)(1<<chm);
	 }
#endif
 ...
 //    IRQ_IntClr(int_id);
	 ...
	 //set configration
	 TAU_REG_TAUBnCMORm(TAUB_BASEADDR[un],chm)=TAUB_CLOCK_SELECT_CK(tauChanlCfg[un][chm])|TAUB_TRIGGER_SELECT(E_TRIG_TIN_VALID)
											 |TAUB_CAPTURE_UPDATE(E_CMOR_UPDATE1) |(E_MODE_CAPTURE |E_MD0_NOINT);  //interval timer func
	 TAU_REG_TAUBnCMURm(TAUB_BASEADDR[un],chm)=edge;  
	 TAU_REG_TAUBnTOE(TAUB_BASEADDR[un])  &=(~(LOGIC_1<<chm)); //set 0,disable output.
	 //Simultaneous rewrite reg cannot be used
	 TAU_REG_TAUBnRDE(TAUB_BASEADDR[un]) &=(~(LOGIC_1<<chm)); //0: Disables simultaneous rewrite
	 TAU_REG_TAUBnRDS(TAUB_BASEADDR[un]) &=(~(LOGIC_1<<chm));
	 TAU_REG_TAUBnRDM(TAUB_BASEADDR[un]) &=(~(LOGIC_1<<chm));
	 TAU_REG_TAUBnRDC(TAUB_BASEADDR[un]) &=(~(LOGIC_1<<chm));
...
/******************************************************************************
* Function Name : TAUJ_IntervalTimer_Set
* Description	 : This function initialize TAUJ (ch.0).
* Parameter   : ck  prescaler clock
*					   count  CDR register value.
*					   isOut timer output enable or not
*					   isLoop 
* Return Value  : none
******************************************************************************/
void TAUJ_IntervalTimer_Set(TAU_CHANNEL chm, CMOR_CKTYPE ck, U32 count, BOOL isOut,BOOL isLoop)
{
	 U8 int_id;
	 TAU_UNIT un=TAUJ0_U3;
	 
	 assert_param(IS_RANGE(chm,TAU_CH0,TAU_CH3));
	 ...
	 IRQ_IntClr(int_id);
	  ...
	  b31:b 0			TAUJnCDR	- Data register for capture/compare values	*/
	  TAU_REG_TAUJnCDR(TAUB_BASEADDR[un], chm) =count;
	  ...
	  b 0				TAUJnMD0	- Specifies whether INTTAUJnIm is generated
									  ...
									  a start trigger is entered) or not.		   - INTTAUDnIm is not generated.
																					 Set to 0'b */
	 TAU_REG_TAUJnCMOR(TAUB_BASEADDR[un], chm) = TAUB_CLOCK_SELECT_CK(ck)|(E_MODE_INTERVAL_TIMER|E_MD0_NOINT); ; 
	 ...
	 TAU_REG_TAUJnCMUR(TAUB_BASEADDR[un], chm) =E_TAUB_TIS_EDGE_FALLING;
 
	 /* The simultaneous rewrite registers (TAUDnTOM, TAUDnTOC, TAUDnTOL, TAUDnTDE , TAUDnTDM , TAUDnTDL
		TAUDnTRE, TAUDnTRO, TAUDnTRC, TAUDnTME) cannot be used with the Interval Timer Function.
		Therefore, these registers must be set to 0. */
	 TAU_REG_TAUJnTOM(TAUB_BASEADDR[un]) &=TAUB_OUTPUT_INDEPENDENT(chm); //0 independent
	 TAU_REG_TAUJnTOC(TAUB_BASEADDR[un]) &=TAUB_OUTPUT_VALUE_0(chm); //0 low
	 TAU_REG_TAUJnTOL(TAUB_BASEADDR[un]) &=(~(LOGIC_1<<chm));
	 ...
	 if(isOut)
	   TAU_REG_TAUJnTOE(TAUB_BASEADDR[un])	|=(LOGIC_1<<chm); //set 1,enable.
	 else
	   TAU_REG_TAUJnTOE(TAUB_BASEADDR[un])	&=(~(LOGIC_1<<chm)); //set 0,disable output.
 
 
	 /* The simultaneous rewrite registers (TAUDnRDE, TAUDnRDS, TAUDnRDM, and TAUDnRDC)
		cannot be used with the Interval Timer Function. Therefore, these registers must be set to 0. */
	 TAU_REG_TAUJnRDE(TAUB_BASEADDR[un]) &=(~(LOGIC_1<<chm)); //0: Disables simultaneous rewrite
	 TAU_REG_TAUJnRDM(TAUB_BASEADDR[un]) &=(~(LOGIC_1<<chm));
	 
	 IRQ_IntEn(int_id); 	  /* Enable interrupt			*/	  
	 allTauChannelCtrl[un][chm].isLoop=isLoop;
	 allTauChannelCtrl[un][chm].isUsed=TRUE;
}
...
* 		   isOut  timer output enable or not
* 		   isLoop timer operating cyclic or not. 
*  Returns	 : None
***********************************************************************/
BOOL TAU_Timer_Init(TAU_UNIT un, TAU_CHANNEL chm, U32 MS_count, BOOL isOut, BOOL isLoop)
{
	U32 src_fre=0;  
	U64 count=0;
	...
	if(!TAU_GetClolckFre_Channel(un, chm, &src_fre))
	  return FALSE;
	  ...
	  */	
	count= (U64)src_fre*MS_count/1000 -1;
	if( (TAUJ0_U3==un) 
#if(MCU_PIN>=PIN100)
		 ||(TAUJ1_U4==un)
#endif
	...
	if(un>=TAUJ0_U3)
	TAUJ_IntervalTimer_Set(chm,(CMOR_CKTYPE)tauChanlCfg[un][chm], (U32)count, isOut,isLoop);
	else
	TAU_IntervalTimer_Set(un, chm,(CMOR_CKTYPE)tauChanlCfg[un][chm],(U16) count, isOut,isLoop);
...
U16 TAU_GetSystemTick_10us(void)
{
 ...
 BOOL getHighValueData(U64 highdata)
 {
	 BOOL radarIndexFlag = FALSE;
	 if(TAUBdataBK != highdata)
	 {
		 TAUBDiffdatavalue = highdata - TAUBdataBK;
		 TAUBdataBK = highdata;
		 if((TAUBDiffdatavalue >=  1)&&(TAUBDiffdatavalue <=	300))//56-66
		 {
			 radarIndexFlag = TRUE;
		 }
		 else 
		 {
			 radarIndexFlag = FALSE;
		 }
	 }

	 return radarIndexFlag;
 }	 


void ISR_TAUD0I1func(void) 
{
	if(tau_func_callback[TAUD0_U2][TAU_CH1]!=NULL)
      tau_func_callback[TAUD0_U2][TAU_CH1](0);

    if(!(allTauChannelCtrl[TAUD0_U2][TAU_CH1].isLoop))
      TAU_REG_TAUJnTT(TAUB_BASEADDR_1(TAUD0_U2)) |= (1<<TAU_CH1);

}

 
#if 0 // hxq modify to uss
...
void TAUBn_Capture_Cmd_Switch(TAU_UNIT un,TAU_CHANNEL chm, FUNCTION_STATE state)
{
    if(state)
    {
                   TAU_REG_TAUBnTS(TAUB_BASEADDR[un]) |=((U16)(1<<chm));
    }
    else
    {
        TAU_REG_TAUBnTT(TAUB_BASEADDR[un])   |=((U16)(1<<chm));
    }
}
