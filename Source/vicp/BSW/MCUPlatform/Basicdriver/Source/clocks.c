...
#define WDTNMIF            (*(volatile unsigned long  *)0xFFC00000u)
#define WDTNMIFC           (*(volatile unsigned long  *)0xFFC00008u)

#define EEPRDCYCL          (*(volatile unsigned char  *)0xFFC59810u)
...
{
#ifdef IC_RH850_F1K
    CLOCK_SET_STPMSK(CLKCTLMOSCSTPM, isMask);
#else
...
{  
#ifdef IC_RH850_F1K
    CLOCK_STOP(WPROTRPROTCMD0, WPROTRPROTS0, CLKCTLMOSCE, CLKCTLMOSCS);
#else
...
void Clock_MainOsc_Start( MOSCC_AMPSEL_MODE gain, U32 stab )
{
    ...
    if( !(CLKCTLMOSCS&CLOCK_STATUS_ACTIVE ))
    {
      CLKCTLMOSCC = (U32)gain|0x04u;   //Set amplification gain
      
      CLKCTLMOSCST=(U32)stab;//Set MOsc stabilization time register

      /* start MOSC */

      PROTECTED_WRITE(WPROTRPROTCMD0, WPROTRPROTS0, CLKCTLMOSCE, CLOCK_START_TRIGGER);

      /* Wait for stable MOSC*/
      while ( !(CLKCTLMOSCS &CLOCK_STATUS_ACTIVE) ); 
    }
...
#ifdef IC_RH850_F1K
    CLOCK_STOP(WPROTRPROTCMD0, WPROTRPROTS0, SOSCE, SOSCS);
#else
...
#ifdef IC_RH850_F1K
    if( (SOSCS&CLOCK_STATUS_ACTIVE) !=CLOCK_STATUS_ACTIVE)  //noreg
    {        
      /* Set Osc stabilization time register ,related to HSI/LSI */
      SOSCST=(U32)stab;  //noreg

      PROTECTED_WRITE(WPROTRPROTCMD0, WPROTRPROTS0, SOSCE, CLOCK_START_TRIGGER);  //noreg

      /* Wait for stable MainOsc*/
      while ( !(SOSCS &CLOCK_STATUS_ACTIVE) ); //noreg
    }
#else
...
{
#ifdef IC_RH850_F1K
    CLOCK_SET_STPMSK(CLKCTLROSCSTPM, isMask);
#else
...
{
#ifdef IC_RH850_F1K
    CLOCK_STOP(WPROTRPROTCMD0, WPROTRPROTS0, CLKCTLROSCE, CLKCTLROSCS);  /* noreg */
#else
...
void Clock_HSIOsc_start(void)
{
  /*Check if MainOsc is stopped*/
#ifdef IC_RH850_F1K
  if(!(CLKCTLROSCS&CLOCK_STATUS_ACTIVE))
  {  
      /* Start MainOsc (without changing STPMK bit) */
      PROTECTED_WRITE(WPROTRPROTCMD0, WPROTRPROTS0, SOSCE, CLOCK_START_TRIGGER);
      /* Wait for stable MainOsc*/
      while ( !(CLKCTLROSCS &CLOCK_STATUS_ACTIVE) );     
  }
  else 
  {
      /*manul stop needed */
...
#ifdef IC_RH850_F1K
    //Clock_PLL_Stop();
    if( !(CLKCTLPLLS&CLOCK_STATUS_ACTIVE) )
    {
      /* PLLk control register (PLLC)
           PLLC[12:11]:Input frequency M[1:0],  Mr val=M[1:0]+1
           PLLC[5:0]:Division ratioN[5:0], Nr val=N[2:0]+1 */
      CLKCTLPLLC=pllc|0x00010300u; 
      /* Start PLL0  */ 
      PROTECTED_WRITE(WPROTRPROTCMD1, WPROTRPROTS1, CLKCTLPLLE, CLOCK_START_TRIGGER);

      /* wait for stable PLL0 */
      while ( !(CLKCTLPLLS&CLOCK_STATUS_ACTIVE) ) {};
    }
#else
    /*Clock_PLL_Stop();*/
...
{  
#ifdef IC_RH850_F1K
    CLOCK_STOP(WPROTRPROTCMD1, WPROTRPROTS1, CLKCTLPLLE, CLKCTLPLLS);  
#else
...
#ifdef IC_RH850_F1K
    PROTECTED_WRITE(WPROTRPROTCMD0, WPROTRPROTS0, CLKCTLCKSC_AWDTAD_CTL, clkdiv);
    while ( CLKCTLCKSC_AWDTAD_ACT != clkdiv )
    {
...
{
#ifdef IC_RH850_F1K
    CLOCK_SET_STPMSK(CLKCTLCKSC_AWDTAD_STPM, isMask);
#else
...
#ifdef IC_RH850_F1K
    PROTECTED_WRITE(WPROTRPROTCMD0, WPROTRPROTS0, CLKCTLCKSC_ATAUJS_CTL, sourcClk);
    while ( CLKCTLCKSC_ATAUJS_ACT != sourcClk )
    {
        /* Wait for set successful */
    }

    /* Divided Clock Selection */
    PROTECTED_WRITE(WPROTRPROTCMD0, WPROTRPROTS0, CLKCTLCKSC_ATAUJD_CTL, clkdiv);
    while ( CLKCTLCKSC_ATAUJD_ACT != clkdiv )
    {
        /* Wait for set successful */
    }    
#else	
...
{
#ifdef IC_RH850_F1K
    CLOCK_SET_STPMSK(CLKCTLCKSC_ATAUJD_STPM, isMask);
#else	
...
void Clock_ARTCA_Set(ARTCA_SRCCLK_TYPE sourcClk, ARTCA_DIV_TYPE clkdiv)
{
    ...
    /* Source Clock Selection */
#ifdef IC_RH850_F1K
    PROTECTED_WRITE(WPROTRPROTCMD0, WPROTRPROTS0, CKSC_ARTCAS_CTL, sourcClk);  //noreg
    ...
    /* Divided Clock Selection */
    PROTECTED_WRITE(WPROTRPROTCMD0, WPROTRPROTS0, CKSC_ARTCAD_CTL, clkdiv);  //noreg
...
#ifdef IC_RH850_F1K
    /* Source Clock Selection */
    PROTECTED_WRITE(WPROTRPROTCMD0, WPROTRPROTS0, CLKCTLCKSC_AADCAS_CTL, sourcClk);
    while ( CLKCTLCKSC_AADCAS_ACT != sourcClk )
    {
        /* Wait for set successful */
    }

    /* Divided Clock Selection */
    PROTECTED_WRITE(WPROTRPROTCMD0, WPROTRPROTS0, CLKCTLCKSC_AADCAD_CTL, clkdiv);
    while ( CLKCTLCKSC_AADCAD_ACT != clkdiv )
    {
        /* Wait for set successful */
    } 
#else
...
{
#ifdef IC_RH850_F1K
    CLOCK_SET_STPMSK(CLKCTLCKSC_AADCAD_STPM, isMask);
#else
...
{
#ifdef IC_RH850_F1K
    assert_param(IS_RANGE(sourcClk,E_AFOUTS_DIS,E_AFOUTS_PPLLCLK4_SAME));
    /* Divided Clock Selection */
    PROTECTED_WRITE(WPROTRPROTCMD0, WPROTRPROTS0, CLKCTLCKSC_AFOUTS_CTL, sourcClk);
    while ( CLKCTLCKSC_AFOUTS_ACT != sourcClk )
    {
        /* Wait for set successful */
    }    
#else
...
{
#ifdef IC_RH850_F1K
    CLOCK_SET_STPMSK(CLKCTLCKSC_AFOUTS_STPM, isMask);
#else
...
#ifdef IC_RH850_F1K
    /* Source Clock Selection */
    PROTECTED_WRITE(WPROTRPROTCMD1, WPROTRPROTS1, CLKCTLCKSC_CPUCLKS_CTL, sourcClk);
    while ( CLKCTLCKSC_CPUCLKS_ACT!=sourcClk )
    {
        /* Wait for CLKCTLCKSC_CPUCLKS_CTL to set. */
    }

    /* Divided Clock Selection */
    PROTECTED_WRITE(WPROTRPROTCMD1, WPROTRPROTS1, CLKCTLCKSC_CPUCLKD_CTL, clkdiv);
    while ( CLKCTLCKSC_CPUCLKD_ACT != clkdiv )
    {
    }
#else
    /* Source Clock Selection */
...
***********************************************************************/
#ifdef IC_RH850_F1K
void Clock_IPERI1_Set(IPERI1_SRCCLK_TYPE sourcClk)
{
    assert_param(IS_RANGE(sourcClk,E_IPERI1S_DISABLE,E_IPERI1S_PPLLCLK));
    /* Source Clock Selection */
    PROTECTED_WRITE(WPROTRPROTCMD1, WPROTRPROTS1, CLKCTLCKSC_IPERI1S_CTL, sourcClk);
    while ( CLKCTLCKSC_IPERI1S_ACT != sourcClk )
    {
    }
}
#else
...
***********************************************************************/
#ifdef IC_RH850_F1K
void Clock_IPERI2_Set(IPERI2_SRCCLK_TYPE sourcClk)
{
    assert_param(IS_RANGE(sourcClk,E_IPERI2S_DISABLE,E_IPERI2S_PPLLCLK2));

    /* Source Clock Selection */
    PROTECTED_WRITE(WPROTRPROTCMD1, WPROTRPROTS1, CLKCTLCKSC_IPERI2S_CTL, sourcClk);
    while ( CLKCTLCKSC_IPERI2S_ACT != sourcClk )
    {
    }
}
#else
...
{
#ifdef IC_RH850_F1K
    assert_param(IS_RANGE(sourcClk,E_ILIN_DIS,E_ILIN_HSI));
    assert_param(IS_RANGE(clkdiv,E_ILIN_DIV1,E_ILIN_DIV8));
    /* Source Clock Selection */
    PROTECTED_WRITE(WPROTRPROTCMD1, WPROTRPROTS1, CLKCTLCKSC_ILINS_CTL, sourcClk);
    while ( CLKCTLCKSC_ILINS_ACT != sourcClk )
    {
        /* Wait for set successful */
    }
    ...
    {
        /* Divided Clock Selection */
        PROTECTED_WRITE(WPROTRPROTCMD1, WPROTRPROTS1, CLKCTLCKSC_ILIND_CTL, clkdiv);
        while ( CLKCTLCKSC_ILIND_ACT != clkdiv )
        {
            /* Wait for set successful */
        }
    }
...
{
#ifdef IC_RH850_F1K
    CLOCK_SET_STPMSK(CLKCTLCKSC_ILIND_STPM, isMask);
#else
...
void Clock_IADCA_Set(AADCA_SRCCLK_TYPE sourcClk, AADCA_DIV_TYPE clkdiv)
{
    ...
    /* Source Clock Selection */
    PROTECTED_WRITE(WPROTRPROTCMD1, WPROTRPROTS1, CKSC_IADCAS_CTL, sourcClk);  //noreg
    while ( CKSC_IADCAS_ACT != sourcClk )  //noreg
    {
        /* Wait for set successful */
    }

    /* Divided Clock Selection */
    PROTECTED_WRITE(WPROTRPROTCMD1, WPROTRPROTS1, CKSC_IADCAD_CTL, clkdiv);  //noreg
    while ( CKSC_IADCAD_ACT != clkdiv )  //noreg
    {
        /* Wait for set successful */
    } 
#else
...
{
#ifdef IC_RH850_F1K
    assert_param(IS_RANGE(sourcClk,E_ICANS_DIS,E_ICANS_PPLLCLK_DEF));
    assert_param(IS_RANGE(clkdiv,E_ICANOSCD_DIS,E_ICANOSCD_MOSC2));

    /* Source Clock Selection */
    PROTECTED_WRITE(WPROTRPROTCMD1, WPROTRPROTS1, CLKCTLCKSC_ICANS_CTL, sourcClk);
    while ( CLKCTLCKSC_ICANS_ACT != sourcClk )
    {
        /* Wait for set successful */
    }

    /* Divided Clock Selection */
    PROTECTED_WRITE(WPROTRPROTCMD1, WPROTRPROTS1, CLKCTLCKSC_ICANOSCD_CTL, clkdiv);
    while ( CLKCTLCKSC_ICANOSCD_ACT != clkdiv )
    {
        /* Wait for set successful */
    }  
#else
...
{
#ifdef IC_RH850_F1K
    CLOCK_SET_STPMSK(CLKCTLCKSC_ICANS_STPM, isMask);
#else
...
{
#ifdef IC_RH850_F1K
    CLOCK_SET_STPMSK(CLKCTLCKSC_ICANOSCD_STPM, isMask);
#else
...
#ifdef IC_RH850_F1K
    /* Source Clock Selection */
    PROTECTED_WRITE(WPROTRPROTCMD1, WPROTRPROTS1, CLKCTLCKSC_ICSIS_CTL, sourcClk);
    while ( CLKCTLCKSC_ICSIS_ACT != sourcClk )
    {
        /* Wait for set successful */
    }
#else
...
void Clock_IICS_Set(IIIC_SRCCLK_TYPE sourcClk)
{
#ifdef IC_RH850_F1K
    /* Source Clock Selection */
    PROTECTED_WRITE(WPROTRPROTCMD1, WPROTRPROTS1, CLKCTLCKSC_IIICS_CTL, sourcClk);
    while ( CLKCTLCKSC_IIICS_ACT != sourcClk )
    {
        /* Wait for set successful */
    }
#else
    /* Source Clock Selection */
    PROTECTED_WRITE(PROTCMD1, PROTS1, CLK_REG_IICS_CTL, sourcClk);
    while ( CLK_REG_IICS_ACT != sourcClk )
    {
        /* Wait for set successful */
    }
#endif
...
{  
  	...
  	pllFreq = CGC_PLLC_Nr(EXTERNAL_PLLCCLK_NR)|CGC_PLLC_Mr(EXTERNAL_PLLCCLK_MR);
  	...
  	/* --config clock domain used  CPUCLK and CPLLOUT 
   	CPLLOUT= VCOOUT/6 =80MHZ
   	CPUCLK, source  CPUPLL, divider 1 =80MHZ  */
  	Clock_CPUCLK_Set(E_ICPUCLK_CPLLCLK, (E_ICPUCLK_DIV1|E_ICPUCLK_DIV4_120MHZ));//clock 120M |E_ICPUCLK_DIV4_120MHZ
	//Clock_AFOUT_Set(E_AFOUTS_PPLLCLK4_SAME);									//clock 30M out put 


  	/*clk IPERI1, fre=PPLLCLK=80MHZ .i.e PERI1_CLKDOMAIN_HZ */
  	Clock_IPERI1_Set(E_IPERI1S_PPLLCLK_DEF);
  	/*clk IPERI2, fre=PPLLCLK2=40MHZ .i.e PERI2_CLKDOMAIN_HZ*/
  	Clock_IPERI2_Set(E_IPERI2S_PPLLCLK2_DEF);

  	Clock_SetDomain_ExternalHighClk();
  
  	PROTECTED_WRITE(WPROTRPROTCMD1, WPROTRPROTS1, CLKCTLCKSC_PPLLCLKS_CTL, CKSC_PPLLCLKS_CTL_PPLLOUT);
...
/*  Clock_AWDTA_SetStpmsK(DOMAIN_AWDT_STPMSK);*/
  ...
  Clock_IADCA_Set(DOMAIN_ADCAS1, DOMAIN_ADCAD);
...
#ifdef IC_RH850_F1K
  /*IIC:PPLLCLK by 2 division,stopped  in stand-by mode */
  Clock_IICS_Set(DOMAIN_IICS);
#endif
...
#ifdef IC_RH850_F1K
  /*IIC:disabled */
  Clock_IICS_Set(E_IIICS_DIS);

  /*clk IPERI1*/
  Clock_IPERI1_Set(E_IPERI1S_DISABLE);
  /*clk IPERI2*/
  Clock_IPERI2_Set(E_IPERI2S_DISABLE);
#else
  /*clk IPERI1*/
...
U32 Clock_GetFrequency_ATAUJ(void)
{
    ...
    else
    {
        ...
        if(E_ATAUJS_LSI240KHZ==DOMAIN_ATAUJS)
            ...
            value=(PPLLCLK_NORMAL_HZ/2U)/div;
    }
