...
/***********************************************************************
*  Name        : Ostm1_init
*  Description : int CMP reg,set  operating mode and interruput fo OSTM1,use for XV1000 32bit CNT 
*  Parameter   : opratMode: as OSTMCTL_OPRTYPE
*                        intEnble: Interrupt on CounterStart disabled or enabled
*                        pllclk_hz and tick_per_sec:  setting OSTM cycle to 1ms
*  Returns     : None
***********************************************************************/
void Ostm1_init(U32 pllclk_hz, U32  tick_per_sec, OSTMCTL_OPRTYPE opratMode, BOOL intEnble)
{
   // U32  cnts;

    assert_param(IS_RANGE(opratMode, E_INTVLTIM_MODE, E_FREERUN_MODE));

    OSTM1_REG_EMU= 0x00u;    /*OSTMnEMU:Counter is continued when Debugger takes control. EPC.SVSTOP = 0*/

//  if(IsOSTM1_ENBLED()) 
//        Ostm1_stop();
    OSTM1_REG_CTL=0x02;/* 1:configure operating system timer free running*/
  /* 1:Interrupts when counting starts are enabled
	 2:Free-run compare mode without interrupts
	 3.Free-run compare mode with interrupts
  */
      /*OSTM0 Cycle=1ms*/
//    cnts = ((pllclk_hz/2u/1000u) *tick_per_sec)-1u;	/*vx1000 without cnt */
    /* if timer is running, stop it.in free rum mode be ignored */
//    OSTM1_REG_CMP=cnts;   /*OSTM0 Cycle=(cnts+1)/((cpupll_hz/2)*1000) =tick_per_sec ms=1ms*/
}
...
{
    /* Start the counter */
    OSTM1_REG_TS=E_STARTSTP_EN;
    while(!IsOSTM1_ENBLED())
    { /*wait until OSTM1 goes to stable*/
    }
}
