...
void UART_SendFirstBuf(UART_TYPE uart, U8* pdata);
...
*  Returns     : None
***********************************************************************/
void UART_DMA_Start( UART_TYPE uart, U32 baudrate)
{	
  /* LIN / UART Control Register */
  UART_REG_RLN3nLCUC(GetUartBaseAddress(uart)) = UARTE_RESET_MODE_CAUSED;
...
/***********************************************************************
*  Name        : UART_SendFirstBuf
*  Description : touch dma to send data
*  Parameter   : none
*  Returns     : None
***********************************************************************/
void UART_SendFirstBuf(UART_TYPE uart, U8* pdata)
...
/***********************************************************************
*  Name        : UART_DMA_SendBuf
*  Description :
*  Parameter   : uart->UART_0,UART_1,...UART_7
                 chr->byte
*  Returns     : U8
***********************************************************************/
void UART_DMA_SendBuf( UART_TYPE uart, U8* pdata,U16 datalen )
{
	if(!datalen)
	{
		return;
	...
	if(datalen > 1)
	{
		DMA_Config_SpiTx(UART4_TX_DMA, (U32)(&pdata[1]), datalen-1);
		UART_SendFirstBuf(UART_4,&pdata[0]);//Send First Buf for trigger dma
		DMA_Start(UART4_TX_DMA); //DMA2
		uart_tx_callback[UART_4]();//clear send.isSending 
	}
	else
	{
		UART_SendFirstBuf(UART_4,&pdata[0]);//Send First Buf for trigger dma
