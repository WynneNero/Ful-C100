...
static vuint8 XcpWriteMta( vuint8 size, const BYTEPTR data )
{
  ...
  while ( size > (vuint8)0u ) 
  {
    ...
    if(0 == ((vuint32)xcp.Mta & (vuint32)0x01) )
    {
...
static void XcpFreeDaq( void )
{
  ...
  #if defined ( kXcpMaxEvent ) && ! defined ( XCP_ENABLE_STIM ) && ! defined ( XCP_ENABLE_DAQ_PRESCALER )
  ...
  /* Data Stimulation (STIM) */
    ...
    if ( (DaqListFlags(daq)&(vuint8)DAQ_FLAG_DIRECTION) != 0 ) /* STIM */
    {
        ...
        else
        {
          ...
          *d = (vuint8)0xFFu; /* Invalidate STIM data buffer */
        ...
        #if defined ( XCP_ENABLE_ADDRESS_DOUBLING_SUPPORT )
          {
            ...
            while(n > 0)
            {
              if(0 == ((vuint32)dest & (vuint32)0x01) )
              {
      ...
      for (odt=DaqListFirstOdt(daq);odt<=DaqListLastOdt(daq);odt++)
      {
        ...
        #if defined ( XCP_ENABLE_DAQ_RUNTIME_SUPERVISION )
          if(XCP_OK != ApplXcpRtsSnapshot())
          {
             /* Runtime supervision timeout occured, create overrun condition and exit */
             status |= (vuint8)(XCP_EVENT_DAQ_OVERRUN | XCP_EVENT_DAQ_TIMEOUT);
             /* Queue overflow */
