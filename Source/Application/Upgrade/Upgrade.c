...
void Upgrade_DISPATCHERMSG(U16 commandid, U8* data)
{
    switch(commandid)
    {
        ...
        case NAVI_SWDL_STATUS_EVENT://0x5413
			...
			if((data[0]==SWDL_GPS)&&(data[1]==SWDL_STATE_START))
			{
				SOC_WATCHDOG_SWITCH = FALSE;
				SocManager_HeartBeatUpdateSocSts(TRUE);
			}		
			if((data[0]==SWDL_GPS)&&(data[1]==SWDL_STATE_END))
			{
				SOC_WATCHDOG_SWITCH = TRUE;
				SocManager_HeartBeatUpdateSocSts(FALSE);
			}
			if((data[0]==SWDL_OS)&&(data[1]==SWDL_STATE_START)) //close watchdog when OS is going to updata
			{
				SOC_WATCHDOG_SWITCH = FALSE;
				SocManager_HeartBeatUpdateSocSts(TRUE);
			}		
...
{	
    ...
    */
    if(ERR_OK == HAL_Write(DEV_FDL,(U8*)&upgradeFlag,CAN_UPGRADE_FLAG_LEN_OF_WORD))
	{
