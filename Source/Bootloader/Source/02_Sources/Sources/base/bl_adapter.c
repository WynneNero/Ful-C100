...
void Adpt_InitDelayInfo(bl_SecurityDelay_t *delay)
{
    ...
    {
		delay->attemptCount =GetSecurityAttCounter();
...
{
	static bl_u8_t DelayIsComplete = TRUE;
    BL_DEBUG_ASSERT_NO_RET(delay != BL_NULL_PTR);
    ...
    else
    {
        if(0 == delay->attemptCount)
        {
            ...
            SetSecurityAttCounter(delay->attemptCount);
#endif
        }
		else if(!DelayIsComplete)  //Once delay timer reached, Att_Cnt should reset.
		{
