...
extern unsigned char response_flag_reset;
bl_BootingFlag_t Bootm_CheckBootingRequirement(void)
{
    bl_BootingFlag_t  flag = 0;

    U8 buffer[8];
	//U8 buffer_reset_inboot[8]= {0xa5,0x5a,0xa5,0x5a,0xBB,0xBB,0xBB,0xBB};
    ...
    if(bR)
    {
        bl_Buffer_t test[4] = {0x88,0x88,0x88,0x88};
        //Mem_Write
        FLS_u8WriteDataWait(0xff200000, 0x04, test);

    }
    //Mem_Read
    FLS_u8ReadDataWait(0xff200000, 0x08, buffer);
    getdata = (((bl_u32_t)buffer[0]<<24)+((bl_u32_t)buffer[1]<<16)+((bl_u32_t)buffer[2]<<8)+(bl_u32_t)buffer[3]);
	getdata_rep_flag =  (((bl_u32_t)buffer[4]<<24)+((bl_u32_t)buffer[5]<<16)+((bl_u32_t)buffer[6]<<8)+(bl_u32_t)buffer[7]);
	/* uidp5359
	switch(getdata)
	{
		case data_can_flag:
