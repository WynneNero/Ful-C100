 ...
 *
 *****************************************************************************/
void Rte_SetDownStep(bl_ProgrammingStep_t *step, bl_u32_t block_addr,bl_u8_t mask)
{
	switch(block_addr){
		case FlashDriverAddr:
			step->flash_programming_process_step |= mask; 
			break;
		case McuappAddr:
			step->mcu_programming_process_step  |= mask; 
			break;
		case SocappAddr:
			step->soc_programming_process_step  |= mask; 
			break;
		default:
			break;
...
/****************************************************************************
 *
 *  \details check  the download step dct.
 *
 *  \return None.
 *
 *  \since V2.0.0
 *
 *****************************************************************************/
bl_Return_t Rte_CheckDownStep(bl_ProgrammingStep_t *step, bl_u32_t block_addr,bl_u8_t mask)
{
	 bl_Return_t ret = BL_ERR_NOT_OK;
	 switch(mask){
		case RequestDownload:
			if(block_addr == FlashDriverAddr)
			{
				...
				if((step->flash_programming_process_step & EraseApplicationMemory) == EraseApplicationMemory)
				{
					ret = BL_ERR_OK;
				}
			}
		...
		case TransferData:
			if(block_addr == FlashDriverAddr)
			{
				if((step->flash_programming_process_step &RequestDownload) == RequestDownload)
				{
					ret = BL_ERR_OK;
				}
			}
			else if(block_addr == McuappAddr)
			{
				if((step->mcu_programming_process_step &RequestDownload) == RequestDownload)
				{
					ret = BL_ERR_OK;
				}
			}
			else if(block_addr == SocappAddr)
			{
				if((step->soc_programming_process_step & RequestDownload) == RequestDownload)
				{
					ret = BL_ERR_OK;
				}
			}
		...
		case RequestTransferExit:
			if(block_addr == FlashDriverAddr)
			{
				if((step->flash_programming_process_step &TransferData) == TransferData)
				{
					ret = BL_ERR_OK;
				}
			}
			...
			{
				if((step->mcu_programming_process_step &TransferData) == TransferData)
				{
					ret = BL_ERR_OK;
				}
			}
			...
			{
				if((step->soc_programming_process_step & TransferData) == TransferData)
				{
					ret = BL_ERR_OK;
				}
			}
		...
		case EraseApplicationMemory:
			if((step->flash_programming_process_step & CheckProgrammingChecksum) == CheckProgrammingChecksum)
			{
				ret = BL_ERR_OK;
			}	
			break;
			
		case CheckProgrammingChecksum: 
			if(block_addr == FlashDriverAddr)
			{
				if((step->flash_programming_process_step &RequestTransferExit) == RequestTransferExit)
				{
					ret = BL_ERR_OK;
				}
			}
			...
			{
				if((step->mcu_programming_process_step &RequestTransferExit) == RequestTransferExit)
				{
					ret = BL_ERR_OK;
				}
			}
			...
			{
				if((step->soc_programming_process_step & RequestTransferExit) == RequestTransferExit)
				{
					ret = BL_ERR_OK;
				}
			}
...
/**************************************************************************//**
 *
 *  \details clean  the download step dct.
 *
 *  \return None.
 *
 *  \since V2.0.0
 *
 *****************************************************************************/
void Rte_ClearDownStep(bl_ProgrammingStep_t *step)
{
	step->flash_programming_process_step = 0;
	step->mcu_programming_process_step =0;
	step->soc_programming_process_step = 0;
    return ;
}
