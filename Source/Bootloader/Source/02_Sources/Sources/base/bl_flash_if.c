 ...
 *      05.01.01 | 14/03/2017 | Jie.Zhang            | N/A | Add function to call fcl directly.
******************************************************************************/
#include "csi_if.h"
#include "bl_common.h"
#include "bl_rte.h"
...
#ifdef USE_FCL_SOURCE_CODE

extern void Flash_Ram_Init(bl_u32_t  ram_addr, bl_FlashDrvParam_t* fp) ;
extern  void Flash_Ram_Erase(bl_u32_t  ram_addr, bl_FlashDrvParam_t* fp);
extern void Flash_Ram_Write(bl_u32_t  ram_addr, bl_FlashDrvParam_t* fp) ;



#define FLASHIF_INIT(pp)    Flash_Ram_Init(FLASHIF_DRIVER_STARTADDRESS, pp)//Flash_Init(pp)//FLASH_DRIVER_INIT(FLASHIF_DRIVER_STARTADDRESS,pp)
#define FLASHIF_DEINIT(pp)  Flash_Ram_Deinit(FLASHIF_DRIVER_STARTADDRESS, pp)//FLASH_DRIVER_DEINIT(FLASHIF_DRIVER_STARTADDRESS,pp)
#define FLASHIF_WRITE(pp)   Flash_Ram_Write(FLASHIF_DRIVER_STARTADDRESS, pp)//Flash_Write(pp)//FLASH_DRIVER_WRITE(FLASHIF_DRIVER_STARTADDRESS,pp)
#define FLASHIF_ERASE(pp)   Flash_Ram_Erase(FLASHIF_DRIVER_STARTADDRESS, pp)//Flash_Erase(pp)//FLASH_DRIVER_ERASE(FLASHIF_DRIVER_STARTADDRESS,pp)
...
static bl_Return_t _Flsif_OperateFlash(bl_FlashIfContext_t *fct)
{
    ...
    while (count > 0UL)
    {
        ...
        if (BL_ERR_OK == ret)
        {
            ...
            {
                fct->timeout = 0UL;
                Rte_ReportEvent(RTE_EVENT_TYPE_SYSTEM_WAIT);
              //FlashWriteTimeout();
            }
        }
        else
