...
void Sys_InitClock(void)
{
	...
	CLKCTLMOSCC = 0x02; 	// Set MainOSC Amplication gain for fx = 16MHZ
	CLKCTLMOSCST = 0x0ffff; // Set MainOSC stabilization time to max (8,19 ms)
	protected_write(WPROTRPROTCMD0, WPROTRPROTS0, CLKCTLMOSCE, 0x01);  // Trigger Enable (protected write)
	while ((CLKCTLMOSCS & 0x04) == 0);   // Wait for active MainOSC

    /* Prepare PLL*/
	CLKCTLPLLC = 0x0000083b|0x00010300U; //16 MHz MainOSC -> 80MHz PLL, no SSCG (MR=2,NR=60)
	protected_write(WPROTRPROTCMD1,WPROTRPROTS1,CLKCTLPLLE,0x01);   //enable PLL
	while((CLKCTLPLLS&0x04) != 0x04);    //Wait for active PLL
    
    /* C_ISO_CPUCLK Clock divider */
	protected_write(WPROTRPROTCMD1,WPROTRPROTS1,CLKCTLCKSC_CPUCLKD_CTL,0x01); /* C_ISO_CPUCLK / 1 */
	while((CLKCTLCKSC_CPUCLKD_ACT & 0x07) != 0x01);

     /* PLL -> C_ISO_CPUCLK */
	protected_write(WPROTRPROTCMD1,WPROTRPROTS1,CLKCTLCKSC_CPUCLKS_CTL,0x03); /* C_ISO_CPUCLK = CPLLCLK */
	while((CLKCTLCKSC_CPUCLKS_ACT & 0x03) != 0x03);

#ifdef OPEN_CAN_OPTION
    /* PLL -> RSCAN Clock: PPLLCLK (PLL) */
	protected_write(WPROTRPROTCMD1,WPROTRPROTS1,CLKCTLCKSC_ICANS_CTL,0x02); /* C_ISO_CAN = PPLLCLK */
	while((CLKCTLCKSC_ICANS_ACT & 0x03) != 0x02);

    /* MainOSC/1 -> RSCAN direct Clock: 16 MHz */
    protected_write(WPROTRPROTCMD1,WPROTRPROTS1,CLKCTLCKSC_ICANOSCD_CTL,0x01); /* C_ISO_CANOSC = MainOSC/1 */
    while((CLKCTLCKSC_ICANOSCD_ACT & 0x03) != 0x01);
#endif

#ifdef OPEN_SPI_OPTION
    protected_write(WPROTRPROTCMD1, WPROTRPROTS1, CLKCTLCKSC_ICSIS_CTL, 0x02); /* C_ISO_CANOSC = MainOSC/1 */
    while((CLKCTLCKSC_ICSIS_ACT & 0x03) != 0x02);
#endif
	...
	#ifdef OPEN_SPI_OPTION
    protected_write(PROTCMD1,PROTS1,CKSC_ICSIS_CTL,0x02); 	/* C_ISO_CANOSC = MainOSC/1 */
    while((CKSC_ICSIS_ACT & 0x03) != 0x02);
	#endif
...
void Sys_DeinitClock(void)
{
    ...
    /* Reset RSCAN Clock: PPLLCLK (PLL) */
    protected_write(WPROTRPROTCMD1,WPROTRPROTS1,CLKCTLCKSC_ICANS_CTL,0x03); 
    while((CLKCTLCKSC_ICANS_ACT & 0x03) != 0x03);

    /* Reset RSCAN direct Clock */
    protected_write(WPROTRPROTCMD1,WPROTRPROTS1,CLKCTLCKSC_ICANOSCD_CTL,0x0); 
    while((CLKCTLCKSC_ICANOSCD_ACT & 0x03) != 0x0);
    ...
    /* Reset C_ISO_CPUCLK */
    protected_write(WPROTRPROTCMD1,WPROTRPROTS1,CLKCTLCKSC_CPUCLKS_CTL,0x01); 
    while((CLKCTLCKSC_CPUCLKS_ACT & 0x03) != 0x01);

    /* Reset  MainOsc */
    protected_write(WPROTRPROTCMD0,WPROTRPROTS0,CLKCTLMOSCE,0x02);  /* Trigger disable (protected write)*/
    while ((CLKCTLMOSCS&0x04) == 1);            /* Wait for inactive MainOSC*/
    CLKCTLMOSCC=0x04;                           /*Set MainOSC gain in reset state*/
    CLKCTLMOSCST=0x44C0;                       /*Set MainOSC stabilization time to reset value*/
     
    /* Reset  PLL0*/
    protected_write(WPROTRPROTCMD1,WPROTRPROTS1,CLKCTLPLLE,0x02);   /*disable PLL*/
    while((CLKCTLPLLS&0x04) != 0x0);          /*Wait for inactive PLL*/
    CLKCTLPLLC=0x0001133b;
...
/*
 #ifdef IC_RH850_F1K
    protected_write(WPROTRPROTCMD0,WPROTRPROTS0,RESCTLSWRESA,0x01); 
 #else
