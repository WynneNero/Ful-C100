...
unsigned char receive_spi_data_block = 0;
...
unsigned char wait_soc_startup = FALSE;
unsigned char response_flag_reset = TRUE;
unsigned char updating_mcu_success ;
//unsigned char Programming_flag = FALSE;
unsigned char reprogramming_complete_flag = FALSE;
unsigned char remainCount_backup = 0xFF;
...
unsigned int delay_to_execute_erase;
BOOL FirstIsGetSeed = FALSE;
bl_u8_t FirstGetSeedSucess = TRUE;
extern void set_1101reset_timer(unsigned int timer);
extern void PgrmAttempCounter();
extern void ClearPgrmAttempCounter();
extern void PgrmCounter();
void  SetSecurityAttCounter(bl_u16_t remainCount);
...
#else
#define ADPT_SESSION_RESPONSE2      ((DCM_P2STARTIMEVALUE_app >> 8) & 0xFFu) /*Resolution (ms/bit)=10ms,modify by LT.20160609*/
#define ADPT_SESSION_RESPONSE3      ((DCM_P2STARTIMEVALUE_app >> 0) & 0xFFu) /*Resolution (ms/bit)=10ms,modify by LT.20160609*/
#endif
...
#define ADPT_UDS_SYSTEM_STATUS_MASK     (ADPT_SYSTEM_STATUS_MASK)
...
#define ADPT_UDS_ERASE_INVALUE_MEMORY       (0xFFFFFFFF)
#define ADPT_UDS_ERASE_INVALUE_SIZE         (0xFFFFFFFF)
...
#define ADPT_UDS_34_SERVICE_SIZE_POS        (ADPT_UDS_34_SERVICE_ADDRESS_POS \
                                                + ADPT_MEMORY_ADDR_FORMAT)
#define ADPT_UDS_34_SERVICE_INVALUE_ADDRESS  (0xFFFFFFFF)
#define ADPT_UDS_34_SERVICE_INVALUE_SIZE     (0xFFFFFFFF)
...
#define ADPT_UDS_36_PAGE_BUFFER_SIZE    (512UL)
...
bl_ProgrammingStep_t Upgrade_Step;

bl_u32_t Block_addr;

BOOL SuspendedResponse_flag= FALSE;
BOOL getNewSeedflag = TRUE;
BOOL NRC36_flag = FALSE;
                                       ...
                                       bl_BufferSize_t *respSize)
{
	...
	Rte_ClearDownStatus(&g_DownContext,RTE_SYSFLAG_CHECK_PROPCDT);
    return resCode;
}
                                                     ...
                                                     bl_BufferSize_t *respSize)
{
    ...
    do
    {
        ...
        {  
        	/* Invalid Key */
            remainCount = Adpt_DecreaseAttemptCount(&gs_UdsPrivateData.delay);
			SetSecurityAttCounter(remainCount);
        ...
        {
            /* Valid Key */
            gs_UdsPrivateData.delay.attemptCount =3;
            gs_UdsPrivateData.delay.delayTime =0;
            SetSecurityAttCounter(gs_UdsPrivateData.delay.attemptCount);
        }
                                              ...
                                              bl_BufferSize_t *respSize)
{
	...
	if(Rte_CheckDownStep(&Upgrade_Step,Block_addr,EraseApplicationMemory))
	{
		...
		&&(BL_BE32_TO_MCU(&buffer[ADPT_UDS_ERASE_MEMORY_SIZE_POS])>0))
    {
		...
		{ 
			...
			Secm_StartHash();//g_VerificationOps.Start();
		}
        ...
        {
            buffer[0] = ADPT_ROUTINE_CORRECT_RESULT;
			Rte_SetDownStep(&Upgrade_Step,Block_addr,EraseApplicationMemory);
        }
        else
        {
                                           ...
                                           bl_BufferSize_t *respSize)
{
	...
	if(Rte_CheckDownStep(&Upgrade_Step,Block_addr,CheckProgrammingChecksum))
	{
		 resCode = DCM_E_REQUESTSEQUENCEERROR;
		 return resCode;
	}
	...
	if(diff_data_for_soc)
	{
		ret = g_VerificationOps.Verify(size,buffer);
	}
	else
	{
		__DI();
	...
	if(BL_BE32_TO_MCU(buffer) == 0xE9411BC1u) //fix checksum value for flashdriver
	{
		ret = BL_ERR_OK;  
		Lbm_Clear_FlashDriver_Status(); //clear flash driver block,Not care block status
	}	

    //if(BL_ERR_OK == ret || diff_data_for_soc==1)
    if(BL_ERR_OK == ret)
    {
    	if(Block_addr != FlashDriverAddr)
    	{
			application_invalue_flag=FALSE;
    	}  
        buffer[0] = ADPT_ROUTINE_CORRECT_RESULT;
		Rte_SetDownStep(&Upgrade_Step,Block_addr,CheckProgrammingChecksum);
    }
    else
    {
        buffer[0] = ADPT_ROUTINE_INCORRECT_RESULT;
                                     ...
                                     bl_BufferSize_t *respSize)
{
    ...
    do
    {
		...
		if((BL_BE32_TO_MCU(&buffer[ADPT_UDS_34_SERVICE_ADDRESS_POS])==0)
			||(BL_BE32_TO_MCU(&buffer[ADPT_UDS_34_SERVICE_SIZE_POS])==0)
			||(BL_BE32_TO_MCU(&buffer[ADPT_UDS_34_SERVICE_ADDRESS_POS])==ADPT_UDS_34_SERVICE_INVALUE_ADDRESS)
			||(BL_BE32_TO_MCU(&buffer[ADPT_UDS_34_SERVICE_SIZE_POS])==ADPT_UDS_34_SERVICE_INVALUE_SIZE))
		{
			resCode = DCM_E_REQUESTOUTOFRANGE;
            break;
		}
		...
		validSize = reqSize - 4; //uidq1219:Remove CRC byte size
		Block_addr = addr;
		if(Rte_CheckDownStep(&Upgrade_Step,Block_addr,RequestDownload))
		{
			resCode = DCM_E_UPLOADDOWNLOADNOTACCEPTED;
			return resCode;
		}

		/*set programming process step */
		Rte_SetDownStep(&Upgrade_Step,Block_addr,RequestDownload);

        ret = Adpt_ActivateDownload(&g_DownContext, addr, validSize);
		if(diff_data_for_soc==1) {
			ret = BL_ERR_OK;
			total_length[1]= (validSize & 0xFF000000)>>24;//buffer[6];
			total_length[2]= (validSize & 0x00FF0000)>>16;//buffer[7];
			total_length[3]= (validSize & 0x0000FF00)>>8; //buffer[8];
			total_length[4]= validSize & 0x000000FF; //(buffer[9] - 0x04); //uidq1219:Remove CRC byte size
			}
        if(BL_ERR_OK == ret)
        {
                                     ...
                                     bl_BufferSize_t *respSize)
{
    ...
    do
    {
		...
		//if(application_invalue_flag == FALSE)
		if((Block_addr != FlashDriverAddr)&&(application_invalue_flag == FALSE))
		{
			application_invalue_flag = TRUE;
			bl_application_value_dataflash_maker();
		}
		...
		/*set programming process step */
		Rte_SetDownStep(&Upgrade_Step,Block_addr,TransferData);
		
		if(dataSize != ADPT_UDS_36_PAGE_BUFFER_SIZE)
		{
		//	for(i=dataSize;i<ADPT_UDS_36_PAGE_BUFFER_SIZE;i++)
		//	{
		//		buffer[i] = ADPT_UDS_36_PADDING_VALUE;
		//	}
			dataSize = ADPT_UDS_36_PAGE_BUFFER_SIZE;
		}		
		...
		*respSize = 0x01u;
		...
		else
		{
			...
			Bl_MemCpy(can_to_spi_data,&buffer[1],512);
			 g_VerificationOps.Update(512,can_to_spi_data);
			/*send 0x78.*/
			gs_UdsPrivateData.serviceId = ADPT_UDS_36_SERVICE_ID;
			_Adpt_TimeOutCallBack(ADPT_UDS_36_SERVICE_ID);
		}
                                     ...
                                     bl_BufferSize_t *respSize)
{
	...
	if(Rte_CheckDownStep(&Upgrade_Step,Block_addr,RequestTransferExit))
	{
		resCode = DCM_E_REQUESTSEQUENCEERROR;
		return resCode;
	}
    ...
    else
    {
        ...
        *respSize = 0x00u;
		...
		/*set programming process step */
		Rte_SetDownStep(&Upgrade_Step,Block_addr,RequestTransferExit);
...
/**************************************************************************//**
 *
 *  \details reset g_DownContext .
 *
 *  \since  V5.1.0
 *
 *****************************************************************************/

void Adpt_ResetDownContext()
{
	_Adpt_UdsClearDownInfo(&gs_UdsPrivateData);
	_Adpt_UdsGotoSession(DCM_STATUS_SESSION_DEFAULT);
}
...
bl_u8_t GetSecurityAttCounter()
{
	bl_u8_t counter[4];
	bl_Return_t ret;
	ret = Dm_ReadData(ADPT_SecurityAttempCounter_FLAGID,(bl_Size_t)4u, counter);
	if(BL_ERR_OK != ret)
	{
		counter[0] = 3;
	}
...
void  SetSecurityAttCounter(bl_u16_t remainCount)
{
	...
	if(remainCount_backup != remainCount)
	{
		counter[0]  = remainCount;
		remainCount_backup = remainCount;
		Dm_WriteData(ADPT_SecurityAttempCounter_FLAGID, (bl_Size_t)4, counter);	
	}
}

/**************************************************************************//**
 *
 *  \details system wait callback.
 *
 *  \since  V5.0.0
 *
 *****************************************************************************/
