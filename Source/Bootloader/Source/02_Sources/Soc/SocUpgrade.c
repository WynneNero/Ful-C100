...
U8* fclptr = NULL;
extern const unsigned char FclCode[10890];
#endif
...
extern unsigned char receive_spi_data_block;
...
void SocApp(void)
{
	...
	//			}
			...
			for(i=0;i<4;i++) {
				...
				SocManager_Entry();
				tm_delay_us(60000);
				SocManager_SendMsg(1,0x5204,128,&can_to_spi_data[i*128]);
			}
			receive_spi_data=0;
        ...
        switch(commandid)
        {
            case NAVI_CURRENT_POWER_SATET : // 0x5401
				...
				if(NAVI_FIRST_CONNECT_COMPLETE == data[0])//03
                {
					...
					soc_msg[0] = TO_USER_ON;
	                soc_msg[1] = ON_OFF_NORMAL;                       
					SocManager_SendMsg(TYPE_SERVICE,RADIO_DISPATCH_CHANGE_POWER_MODE,2,&soc_msg);
                }			
            break;
