...
ERR_TYPE swdl_check_first_row_valid(void)
{
    ...
    if(swdl_data_recieved)
    {
        ...
        {
           	get_project_number();	//checksum correct

           	//if(memcmp(project_number_in_local_flash, file_data_project_number, 10) == 0)	//WYJ
           	if(1)
           	{
              	if(check_first_line_xor_value_valid())	//these two variables are the same
                {
                    retval = ERR_OK;	//check xor is correct
                    file_data_error_counter = 0;                    
                }
                else
                {
                    if( ++file_data_error_counter < 3)	//check the xor is wrong
                    {
                        retval = ERR_RETRY;		//try to 3times
                        //navi_pw_request_navi_get_cur_swdl_data();                 
                    }
                    else
                    {
                        retval = ERR_FAILED;	//exceed 3times error, request reset.
        ...
        {
            if( ++file_data_error_counter < 3)		//checksum error
            {
                retval = ERR_RETRY;		//try to 3times
                //navi_pw_request_navi_get_cur_swdl_data();                 
            }
